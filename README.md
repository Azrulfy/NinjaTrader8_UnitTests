...
This repository is an attempt to experiment with an approach to "unit testing" NinjaScript. The idea is to run the NinjaScript code under development regularly on a chart (with just one bar!) so that the OnBarUpdate method can be used to call and run "test code". Please see the attached code samples and ping me if it is not obvious what I am trying to do...
